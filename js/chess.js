...
function Game(){
	var Game = this, squares = $('#board ul li'), selectedSquare = null, gameOver = false,
	cLabels = "ABCDEFGH";
...
//		$(pieces).draggable("disable"); // disables piece dragging for previous player
	...
	function logMove(piece, start, captured){
		...
		end = piece.position.toLowerCase(),
		moveType = (captured == null) ? " - " : " x ",
		pieceType = (piece.type != "knight") ? piece.type.toUpperCase().charAt(0) : "N", move;
		...
		if (piece.type == 'pawn' && piece.row() == piece.endRow) 
			piece.logCell = cell;
	...
	function Player(side){
		...
		this.name = prompt(this.color[0].toUpperCase() + this.color.substr(1) + 
			" side's name:", 'Player ' + ($(Game.Players).size() + 1));
 ...
 self = this, capturedPiece = null;
			...
			if (this.type != 'king' && check(Game.Players[0].King.position, Game.Players[1], capturedPiece)) {
				...
				if (!(this.type == 'rook' && Game.castled)) 
					logMove(this, location, capturedPiece);
			};
		};
	...
	function pawn(color, start){
		...
		this.footprint = function(){
			row = self.row(), col = self.col(), ids = new Array();
			
			if (!self.moved) 
				ids.push(col + (row + 2 * this.inc));
			else 
		...
		this.move = function(destination){
			...
			if (!this.moved && (destination.id.match(4) || destination.id.match(5))) {
				...
				if (prev && prev == 'pawn' && prev.color != color) 
					prev.EP = this;
				if (next && next == 'pawn' && next.color != color) 
					next.EP = this;
			};
	...
	function knight(color, start){
		...
		this.footprint = function(){
			var ids = [], col = cLabels.indexOf(self.col()), row = self.row(), cShift;
	...
	function king(color, start){
		...
		this.move = function(destination){
			...
			if (this.castle() && (destination.id.match('G') || destination.id.match('C'))) {
				...
				if (destination.id.match('G')) {
					var rook = callPiece($('#H' + this.row()).children('img')[0]), dest = destination.previousElementSibling;
					...
					if (destination.id.match('C')) {
						var rook = callPiece($('#A' + this.row()).children('img')[0]), dest = destination.nextElementSibling;
		...
		this.footprint = function(){
			var row = this.row(), column = cLabels.indexOf(this.col()), square, squares = new Array();
	...
	function Legal(piece){
		var footprint = piece.footprint(), color = piece.color, type = piece.type, position = piece.position, rNum = position[1] * 1, C = position[0], cNum = cLabels.indexOf(C) + 1, dest, orig, path, legalIDs = "";
	...
	function vector(start, end, side, capture){
		var sX = cLabels.indexOf(start[0]), eX = cLabels.indexOf(end[0]), sY = start[1] * 1, eY = end[1] * 1, xInc = findInc(sX, eX), yInc = findInc(sY, eY), squareList = '', dest, piece, square;
	...
	function occupied(square_ID){
		if (typeof(square_ID) != 'string') 
			return false;
		if (square_ID.match('#') <= 0) 
			square_ID = '#' + square_ID;
	...
	function writeID(column, row){
		try {
			...
			if (er == 0) 
				...
				if (er == 1) 
					alert('Square ID Error: row is not defined');
				else 
					if (er == 2) 
						alert('Square ID Error: row is less than board index');
					else 
						if (er == 3) 
							alert('Square ID Error: row is greater than board index');
		}
		return '#' + column + row + ","
	};
	...
	function findDiagonal(start, xInc, yInc){
		var x = cLabels.indexOf(start[0]), y = start[1] * 1;
	...
	function inside(square, origin){
		if (square && origin != square && square[1] * 1 >= 1 && square.substr(1) * 1 <= 8 && cLabels.indexOf(square[0]) >= 0) 
			return true;
		else 
			return false;
	};
	...
	function check(square, player, ignore){
		var chk = new Array(), ids, footprint;
